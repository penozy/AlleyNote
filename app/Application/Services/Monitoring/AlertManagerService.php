...
final class AlertManagerService
{
    ...
    public function getActiveAlerts(?AlertSeverity $severity = null): array
    {
        ...
        if ($severity !== null) {
            $alerts = array_filter($alerts, fn($alertData) => 
                AlertSeverity::from($alertData['severity']) === $severity
            );
        }
    ...
    public function getAlertStatistics(): array
    {
        ...
        // 尋找最舊和最新的告警
        usort($activeAlerts, fn($a, $b) => 
            $a->alertedAt->getTimestamp() <=> $b->alertedAt->getTimestamp()
        );
